...
category: te
kind: engineering
purpose: The first half of a two-skill dependency cycle fixture.
description: |
  Use when the test suite needs a dependency cycle. This skill depends on
  uesf-te-cycle-b, which depends back on it, forming a cycle that the validator must
  detect.
triggers:
...
## Overview
Part one of a dependency cycle.
